...
void init(Coordinator &coordinator)
{
    ...
    coordinator.registerComponent<ParallaxeComp>();
}
...
{
    Signature sig;
    auto playerMotionSystem = coordinator.registerSystem<PlayerMotion>();

    sig.set(coordinator.getComponentType<Motion>());
    sig.set(coordinator.getComponentType<Input>());
    coordinator.setSystemSignature<PlayerMotion>(sig);
    playerMotionSystem->init(&coordinator);

    return (playerMotionSystem);
}

std::shared_ptr<GestButton> initGestButtonSystem(Coordinator &coordinator, sf::RenderWindow *window)
{
    Signature sig;
    auto gestButtonSystem = coordinator.registerSystem<GestButton>();

    sig.set(coordinator.getComponentType<Sprite>());
    sig.set(coordinator.getComponentType<Clickable>());

    coordinator.setSystemSignature<GestButton>(sig);
    gestButtonSystem->init(&coordinator, window);

    return (gestButtonSystem);
}
...
{
    Signature sig;
    auto textBoxSystem = coordinator.registerSystem<TextBox>();

    sig.set(coordinator.getComponentType<Sprite>());
    sig.set(coordinator.getComponentType<Clickable>());
    sig.set(coordinator.getComponentType<Text>());

    coordinator.setSystemSignature<TextBox>(sig);
    textBoxSystem->init(&coordinator, window);
...
{
    Signature sig;
    auto printTextSystem = coordinator.registerSystem<PrintText>();

    sig.set(coordinator.getComponentType<Text>());

    coordinator.setSystemSignature<PrintText>(sig);
    printTextSystem->init(&coordinator, window);

    return (printTextSystem);
}
...
{
    Signature sig;
    auto createSettingsSystem = coordinator.registerSystem<Settingsys>();

    sig.set(coordinator.getComponentType<SettingsStr>());
    coordinator.setSystemSignature<Settingsys>(sig);
    createSettingsSystem->init(&coordinator);

    return (createSettingsSystem);
}

std::shared_ptr<MovingBar> initMovingBarSystem(Coordinator &coordinator, sf::RenderWindow *window, sf::Sound *sound)
{
    Signature sig;
    auto MovingBarsSystem = coordinator.registerSystem<MovingBar>();

    sig.set(coordinator.getComponentType<Bar>());
    sig.set(coordinator.getComponentType<Sprite>());
    sig.set(coordinator.getComponentType<Text>());
    coordinator.setSystemSignature<MovingBar>(sig);
    MovingBarsSystem->init(&coordinator, window, sound);

    return (MovingBarsSystem);
}

std::shared_ptr<Parallaxe> initParallaxeSystem(Coordinator &coordinator, sf::RenderWindow *window)
{
    Signature sig;
    auto ParallaxeSystem = coordinator.registerSystem<Parallaxe>();

    sig.set(coordinator.getComponentType<Sprite>());
    sig.set(coordinator.getComponentType<ParallaxeComp>());
    coordinator.setSystemSignature<Parallaxe>(sig);
    ParallaxeSystem->init(&coordinator, window);

    return (ParallaxeSystem);
}
